...
if (import.meta.vitest) {
  ...
  test('syncTest', async () => {
    ...
    //消えていることの確認
    const deletedlsProcess = ls('onedrive:', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
  ...
  test('conflictTest', async () => {
    ...
    await new Promise<void>((r) => syncProcess.on('close', r));
    /**同期後のリモートのファイル一覧を取得 */
    const beforeRemotelsProcess = ls('onedrive:sync_conflict', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
    ...
    const deleteProcess = rcloneDelete('onedrive:sync_conflict/hoge3', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
    ...
    /**消えていることを確認 */
    const deletedRemotelsProcess = ls('onedrive:sync_conflict', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
    ...
    /**同期したファイルリストを取得 */
    const uploadedRemotelsProcess = ls('onedrive:sync_conflict', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
    ...
    //onedrive:syncの削除
    const deleteRemoteDirectoryProcess = purge('onedrive:sync_conflict', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
