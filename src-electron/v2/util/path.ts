...
export class Path {
  ...
  createWriteStream(encoding: BufferEncoding): Writable<string>;
  createWriteStream(): Writable<Buffer>;
  createWriteStream(encoding?: BufferEncoding) {
    return Writable.fromNodeStream(
      fs.createWriteStream(this.str(), {
        encoding,
      })
    );
  }

  createReadStream(encoding: BufferEncoding): Readable<string>;
  createReadStream(): Readable<Buffer>;
  createReadStream(encoding?: BufferEncoding) {
    return Readable.fromNodeStream(
      fs.createReadStream(this.str(), { encoding })
    );
  }
