...
if (import.meta.vitest) {
  ...
  test('copyFileLocalToRemote', async () => {
    ...
    const deleteProcess: ChildProcess = rcloneDelete('dropbox:sync/hoge3', {
      env: {
        RCLONE_CONFIG: 'src-electron/rclone-sample/rclone.conf',
      },
    });
    await new Promise<void>((r) => deleteProcess.on('close', r));
    const deletedRemotelsProcess = ls('dropbox:sync');
    ...
    test('copyFileRemoteToLocal', async () => {
      const touchProcess: ChildProcess = touch('dropbox:sync/hoge4');
