import { ServerProperty, World } from 'src-electron/api/schema';
...
export const useWorldEditStore = defineStore('worldEditStore', {
  ...
  actions: {
     ...
     */
    setEditer(world: World, saveFunc: () => void, {worldIndex = -1, title = 'ワールド編集'}) {
      this.world = world
     ...
     */
    async updateRows() {
      this.propertyRows = Object.entries(await this.setfirstProperty()).map(([name, value]) => ({ name, value }))
    }
  }
});
