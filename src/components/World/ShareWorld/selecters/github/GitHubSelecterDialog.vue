...
<template>
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <BaseDialogCard
      :title="
        $t('shareWorld.sync', {
          path: `${remoteData.owner}/${remoteData.repo}`,
        })
      "
    ...
    >
      <q-card-section>
        <span class="text-caption">{{
          $t('shareWorld.selectRemote.title')
        }}</span>
        <q-card-actions>
          <SsBtn
            free-width
      ...
      <q-card-section>
        <span class="text-caption">{{
          $t('shareWorld.selectRemote.syncExistWorld')
        }}</span>
        <div v-if="remoteWorlds.length === 0" class="messageField">
          <div
            v-if="loading"
            class="absolute-center messageText row items-center"
          >
            <q-circular-progress
              indeterminate
              rounded
        ...
        <div class="row q-gutter-sm justify-center">
          <template
            v-for="remoteWorld in remoteWorlds"
            :key="remoteWorld.remote.name"
          >
            <WorldItem
