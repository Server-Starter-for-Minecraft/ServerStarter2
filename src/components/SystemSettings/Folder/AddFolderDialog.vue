...
const pickPath = ref(
  prop.containerSettings?.container ?? ('' as WorldContainer)
);
...
function isErrorName(c: WorldContainerSetting) {
  return (
    c.name === inputName.value &&
    inputName.value !== prop.containerSettings?.name
  );
}
...
function isErrorPath(c: WorldContainerSetting) {
  return (
    c.container === pickPath.value &&
    pickPath.value !== prop.containerSettings?.container
  );
}
    ...
    >
      <template #additionalBtns>
        <ss-btn
          color="primary"
          :disable="
            sysStore.systemSettings.container.filter(isErrorContainer).length >
              0 ||
            inputName === '' ||
            pickPath === ''
          "
      ...
      <div class="row q-gutter-md">
        <SsInput
          v-model="inputName"
          :label="$t('others.worldFolder.folderName')"
          autofocus
          class="col"
        />
        <SsBtn
          free-width
          :label="$t('others.worldFolder.selectFolderBtn')"
          @click="pickFolder"
        />
      </div>
