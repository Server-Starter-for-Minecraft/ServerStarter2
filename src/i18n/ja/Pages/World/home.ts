export const jaHome = {
  ...
  versionChange: {
    title: 'サーバーの種類を変更',
    desc: '以前起動したサーバーの種類を変更する操作は，ワールドデータが破損する恐れがあります．\n危険性を理解した上でバージョンの変更を行いますか？',
    okbtn: '危険性を理解して変更する',
  },
  versionDown: {
    title: 'バージョンダウンの確認',
    desc: '以前起動したサーバーのバージョンからバージョンを下げる操作は，ワールドデータが破損する恐れがあります．\n危険性を理解した上でバージョンの変更を行いますか？',
    okbtn: '危険性を理解して変更する',
  },
  serverType: {
    vanilla: 'バニラ (公式)',
  ...
  ngrok: {
    title: 'ポート開放不要化',
    desc: '\
      「ポート開放」と呼ばれる設定をせずに，友人や外部の方をサーバーに招待するための機能です\n\
    ...
    dialog: {
      firstPage: {
        title: 'はじめに',
        desc: '\
          本ソフトはポート開放不要化にNgrokと呼ばれるツールを用います\n\
          アカウントをお持ちでない方はアカウントの新規作成をお願いします\n\
          ※アカウントは有料版もありますが，{0}',
        assumption: '本ソフトは無料アカウントでの利用を想定しています',
        register: 'アカウントを新規作成',
      ...
      secondPage: {
        ...
        howToConnect: {
          title: '接続方法',
          desc: 'ServerStarter2でサーバーを起動すると画面の右上に{0}IPアドレスが表示されます',
          changeNum: '起動ごとに値が変更される',
          connect:
  ...
  deleteWorld: {
    ...
    dialogDesc:
      '\
      {deleteName}のデータは永久に失われ、元に戻すことはできません。\n\
      本当にワールドを削除しますか？',
  },
  error: {
    title: '警告',
